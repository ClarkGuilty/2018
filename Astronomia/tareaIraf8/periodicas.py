...
for grade in K:
    ...
    plt.plot(x,threshold*y, label='grado '+str(grade))
...
for grade in K:
    fit = np.polynomial.polynomial.polyfit(mag,emag,grade)
    y = np.polynomial.polynomial.polyval(x,fit)
    plt.plot(x,2.3*y, label='grado '+str(grade), color='purple')
plt.xlabel("Magnitud",fontsize=20)
plt.ylabel("Dispersión",fontsize=20)
plt.legend()
...
plt.scatter(mag[ii],emag[ii], color = 'r')
plt.scatter(mag[~ii],emag[~ii], color = 'cyan')

plt.savefig('fits2.png',dpi=300)
